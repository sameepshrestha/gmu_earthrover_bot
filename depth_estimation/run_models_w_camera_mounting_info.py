...
def compute_max_visible_depth(image_height, focal_length, camera_height, camera_tilt_deg):
    v0 = image_height / 2.0
    v_max = image_height - 1
    camera_tilt_rad = np.deg2rad(camera_tilt_deg)
    pixel_angle_max = np.arctan2((v_max - v0), focal_length)
    total_angle_max = camera_tilt_rad + pixel_angle_max

    if total_angle_max <= 0:
        return 10.0  # fallback
    return camera_height / np.tan(total_angle_max)
...
def test_single(depth_estimator, directory, depth_reference_val=10.0):
    ...
    # Compute max visible depth based on projection
    max_depth = compute_max_visible_depth(
        image_height=rs_image.shape[0],
        focal_length=525.0,         # adjust if needed
        camera_height=0.42,         # in meters
